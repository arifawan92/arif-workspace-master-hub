import os, zipfile, hashlib  # <-- YE SABSE PEHLE
...
# --- CONFIG - ORGANIZED ---
BASE_DIR = r"D:\Arif Workspace\Projects\peos"
BASE_PATH = os.path.join(BASE_DIR, "Data", "Projects", "KP-Police")
BACKUP_PATH = os.path.join(BASE_DIR, "Reports", "Backup")
EXCEL_PATH = os.path.join(BASE_DIR, "Reports", "Excel")
CHAT_LOG_FILE = os.path.join(BASE_DIR, "Logs", "PEOS_ChatLog.json")
...
ALERT_DAYS = 3
YOUR_WHATSAPP = "923001401022"
MAX_RETRY_ATTEMPTS = 3 # 3 baar try
RETRY_DELAY_SECONDS = 5 # 5 sec wait
TEAM = {
...
def scan_with_retry_limit(scan_function):
    """
    3 baar try karega. Fail ho gaya to ruk jayega.
    """
    attempt = 0
    while attempt < MAX_RETRY_ATTEMPTS:
        attempt += 1
        try:
            result = scan_function() # asal scan yahan chalega
            log(f"Scan Complete. Attempt {attempt}. Success.")
            return result
        except Exception as e:
            msg = f"Scan Error (attempt {attempt}/{MAX_RETRY_ATTEMPTS}): {e}"
            log(msg)
            save_chat_log("ERROR", f"Attempt {attempt}: {str(e)[:40]}")
            if attempt < MAX_RETRY_ATTEMPTS:
                log(f"{RETRY_DELAY_SECONDS} sec baad dobara try...")
                time.sleep(RETRY_DELAY_SECONDS)
            else:
                final_msg = f"ALERT: Scan failed after {MAX_RETRY_ATTEMPTS} attempts. Manual check zaroori hai."
                log(final_msg)
                save_chat_log("ALERT", final_msg)
                return None
...
def safe_scan_peos():
    return scan_with_retry_limit(_real_scan_peos)
...
def export_report():
    if not data_for_export:
        messagebox.showwarning("Warning", "Pehle Scan Now daba do")
        return

    file_path = filedialog.asksaveasfilename(
        initialdir=EXCEL_PATH,
        defaultextension=".xlsx",
        filetypes=[("Excel files", "*.xlsx")],
        initialfile=f"{PROJECT_ID}-Report-{datetime.now().strftime('%Y%m%d')}.xlsx"
    )

    if file_path:
        df = pd.DataFrame(data_for_export[1:], columns=data_for_export[0])
        df.to_excel(file_path, index=False)
        log(f"Excel Report Exported: {file_path}")
        messagebox.showinfo("Success", f"Report Saved:\n{file_path}")

def scan_and_send():
    res = safe_scan_peos()
    if res is None: # 3 baar fail
        log("Auto WhatsApp skip. Scan failed.")
        return
    time.sleep(2)
...
def start_auto():
    ...
    log(f"Auto Pilot ON. Har 1 ghante + 3 Retry Limit active")

# GUI
root = Tk(); root.title(f"{PROJECT_ID} Dashboard v3.1 - Auto Pilot"); root.geometry("720x620")
...
log(f"{PROJECT_ID} v3.1 Started. Auto Pilot + 3 Retry Ready")

root.iconify() # Khulte hi minimize
root.mainloop()